...
- anyModelChange(function, handler): like modelChange but fired when any change is made to the children
- bindAdd(function, handler): function will be called with the first argument as the model that was added to the collection
- bindRemove(function, handler): function called with the first argument as the removed model and second as id or Cid (in case model was disposed early).
