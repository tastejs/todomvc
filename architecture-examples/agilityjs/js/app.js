...
(function() {
	...
	$(function() {
		// cleanup example item already given in the template
		...
		var app = $$({
			model: {completeCount:"0", todoCount:"0",
				toggleAll:false, mainStyle:"", clearControlStyle:""},
			...
			updateCounter: function() {
				...
				this.model.set({
					...
					clearControlStyle: (0 < completeCount ? "display:block" : "display:none")
				});
			},
			// filter handler
			filters: {
				"#/":          function(obj) { return true; },
				"#/active":    function(obj) { return !obj.model.get('complete'); },
				"#/completed": function(obj) { return obj.model.get('complete'); }
			},
			applyFilter: function(hash) {
				var filter = this.filters[hash];
				this.each(function(i,obj) {
					if (filter(obj)) {
						obj.setStatus(obj.model.get('complete') ? 'complete' : '');
					} else {
		...
		// manual filter routing  (not supported by agilityjs)
