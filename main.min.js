	...
	$.fn.persistantPopover = function () {
		...
		return this.each(function () {
			...
			$this.popover({
				...
				animation: false,
				html: true,
				title: this.firstChild.textContent + '<a href="site-assets/' + $this.data('source') + '">Website</a>'
			});
		})
		.mouseenter(function () {
...
</=e.top?!1:null!=h&&e.top+this.$element.height()></=e[a+1])&&this.activate(f[a])},b.prototype.activate=function(b){this.activetarget=b,a(this.selector).parentsuntil(this.options.target,".active").removeclass("active");var></=e[0])return></g.left?e.left=g.left-j:k></o.left?"right":g,e.removeclass(m).addclass(g)}var></0?"bottom":"right"==g&&j.right+k></i.length-1&&j++,~j||(j=0),i.eq(j).trigger("focus")}}}};var></div></div></=0&&a.offsetheight<=0},n.expr.filters.visible=function(a){return!n.expr.filters.hidden(a)};var></0&&(d+=e+"></c.length;b++)a=c[b],a()||c[b]!==a||c.splice(b--,1);c.length||n.fx.stop(),la=void></iframe></$1></$1></([\w:]+)></b.length&&g.push({elem:this,handlers:b.slice(h)}),g},props:"altkey></0&&"on"+q,b=b[n.expando]?b:new></c?n.queue(this[0],a):void></11&&(g?g.index(c)></"===a[0]&&"></[\w\w]+></\></(\w+)\s*\></b;)a.push(d);return></6)return!1;return!0},parent:function(a){return!d.pseudos.empty(a)},header:function(a){return></31,d={}.hasownproperty,e=[],f=e.pop,g=e.push,h=e.push,i=e.slice,j=function(a,b){for(var>