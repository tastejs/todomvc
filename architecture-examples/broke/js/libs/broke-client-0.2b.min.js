...
;(function(__global__, undefined){
        ...
        ,initProject= function(){
            ...
            settings.INSTALLED_APPS= builtins.map(settings.INSTALLED_APPS, function(){
                ...
                builtins.forEach(app.models, function(){
                    ...
                    for(name in broke.conf.settings.DATABASES){
                        dbEngine = builtins.getattr(broke.conf.settings.DATABASES[name].ENGINE);
                        if(dbEngine.hasOwnProperty('initTableForModel')) {
                            dbEngine.initTableForModel(this);
                        }
                    }
    ...
    broke.extend(broke.DOM, {
        ...
        ,text: function(element, text){
            jQuery(element).text(text.toString());
    ...
    broke.conf.settings= {
        ...
        ,DATABASES: {
            ...
            ,'localStorage': {
                NAME: ''
                ,ENGINE: 'broke.db.engines.LocalStorageEngine'
                ,USER: ''
                ,PASSWORD: ''
            }
            ,'remote': {
    ...
    broke.events= {
        ...
        ,postCreate: function(klass, fn){
            var
                eventName= 'broke.' + klass.__name__.toLowerCase() + '.post_create'
            ;

            events.addListener(window, eventName, fn);
        }
        ,preUpdate: function(klass, fn){
            var
                eventName= 'broke.' + klass.__name__.toLowerCase() + '.pre_update'
            ;

            events.addListener(window, eventName, fn);
        }
        ,postUpdate: function(klass, fn){
            var
                eventName= 'broke.' + klass.__name__.toLowerCase() + '.post_update'
            ;

            events.addListener(window, eventName, fn);
        }
        ,postDelete: function(klass, fn){
            var
                eventName= 'broke.' + klass.__name__.toLowerCase() + '.post_delete'
                ;

            events.addListener(window, eventName, fn);
        }
        ,preDelete: function(klass, fn){
            var
                eventName= 'broke.' + klass.__name__.toLowerCase() + '.pre_delete'
            ;
...
;(function(__global__, undefined){
    ...
    var
        remoteModels= []
        ,remoteModelsForRead= remoteModels
        ,remoteModelsForWrite= remoteModels
    ;
    ...
    var
        ...
        ,cleanData= function(kwargs, model){
            ...
            if(defaultPistonAttributeFieldsHolder && kwargs[defaultPistonAttributeFieldsHolder]) {
                ...
                delete result.fields.pk;
            }

            return result;
        }
        ,DOMlocalStorage= (function(){
    ...
    broke.storages= {
        ...
        ,JSONSchema: {
            ...
            ,set: function(tableName, kwargs) {
                ...
                if(!kwargs.pk) {
                    kwargs.pk= broke.storages.getUniqueId();
                }
        ...
        ,localStorage: {
            ...
            ,initTableForModel: function(model, objects){
                ...
                if(!this.getTable(tableName)) {
                    DOMlocalStorage.set(tableName, objects || []);
                }

                // init pk index for this table
                broke.storages.JSONSchema.index[tableName]= {};

                return DOMlocalStorage.get(tableName);
            }
            ,getTable: function(tableName){
                return DOMlocalStorage.get(tableName);
            }
            ,saveTable: function(tableName, dbTable){
                return DOMlocalStorage.set(tableName, dbTable);
            }
            ,get: function(pk, tableName, andPosition){
                var
                    dbTable = this.getTable(tableName)
                    ,i
                    ,dbTableLen = dbTable.length
                    ,record
                ;

                for(i=0; i < dbTableLen; i++){
                    record = dbTable[i];
            ...
            ,set: function(tableName, kwargs){
                var
                    dbTable= broke.storages.localStorage.getTable(tableName)
                    ,i= dbTable.length
                    ,objAndPosition= broke.storages.localStorage.get(kwargs.pk, tableName, true)
                    ,model= JSONSchemaModelStorageMapping[tableName]
                    ,obj = objAndPosition[0]
                    ,position = objAndPosition[1]
                ;
                ...
                broke.storages.localStorage.saveTable(tableName, dbTable);
    ...
    LocalEngine= BaseEngine.create({
        ...
        ,update: function(callback){
            this.args.insert = this.args.update;
            return this.insert(callback);
        }
        ,'delete': function(callback){
            this.storage['delete'](this.model.getTableName(), this.object.pk);
        }
        ,orderBy: function(){
            // TODO
            /*
    ...
    LocalStorageEngine = LocalEngine.create({
        __name__: "broke.db.engines.LocalStorageEngine"
        ,storage: broke.storages.localStorage
    });
    LocalStorageEngine.initTableForModel= function(model, objects){
        return this.prototype.storage.initTableForModel(model, objects);
    };
    ...
    QuerySet= Class.create({
        ...
        ,create: function(args, callback){
            var
                ...
                ,postCreate= function(obj){
                    broke.DOM.events.trigger(window, 'broke.' + querySet.model.__name__.toLowerCase() + '.post_create', [obj]);
                    if(builtins.isFunction(callback)) {
        ...
        ,getOrCreate: function(args, callback){
            ...
            try {
                return this.get(args, getOrCreateCallback);
            } catch(e) {
                if(e.name == 'DoesNotExist') {
                    created = true;
                    return this.create(args, getOrCreateCallback);
                }
...
;(function(undefined){
    ...
    Class.create({
        ...
        ,__init__: function(__class__){
            ...
            __class__._meta = { fields: {} };
            __class__._meta.appLabel= __class__.appLabel;
            __class__._meta.modelName= __class__.__name__.toLowerCase();
...
//                }
        ...
        ,getTableName: function(){
            if(!this._tableName){
                this._tableName= broke.conf.settings.DB_PREFIX + [this.appLabel, this.__name__.toLowerCase()].join('_');
            }
    ...
    Model= Class.create({
        ...
        ,update: function(fields, updateDOM){
            ...
            broke.DOM.events.trigger(window, 'broke.' + this.__class__.__name__.toLowerCase() + '.pre_update', [this, fields]);
            ...
            broke.DOM.events.trigger(window, 'broke.' + this.__class__.__name__.toLowerCase() + '.post_update', [this, fields]);
        ...
        ,save: function(saveSettings, callback){
            ...
            if(saveSettings.commit) {
                return engineForWrite({
                    ...
                    broke.DOM.events.trigger(window, 'broke.' + className + '.post_' + engineOperation, [object, saveSettings]);
