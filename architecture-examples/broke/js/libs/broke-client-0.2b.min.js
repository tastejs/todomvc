    ...
    broke.storages= {
        ...
        ,localStorage: {
            ...
            ,initTableForModel: function(model, objects){
                ...
                if(DOMlocalStorage.get(tableName) === undefined) {
                ...
                DOMlocalStorageMapping[tableName]= model;

                return DOMlocalStorage.get(tableName);
            }
            ,getTable: function(tableName){
                return DOMlocalStorageMapping.get(tableName);
            }
            ,get: function(){
                // TODO: improve
                broke.storages.localStorage.index[tableName]= broke.storages.localStorage.index[tableName] || {};

                return broke.storages.localStorage.index[tableName][pk];
            }
            ,set: function(tableName, kwargs){
                var
                    dbTable= broke.storages.localStorage.getTable(tableName)
                    ,i= dbTable.length
                    ,obj= broke.storages.localStorage.get(kwargs.pk, tableName)
                    ,model= JSONSchemaModelStorageMapping[tableName]
                    ,position
                ;
                ...
                if(kwargs.pk == 'auto') {
                    kwargs.pk= 'new_' + broke.storages.localStorage.autoPk;
                    broke.storages.localStorage.autoPk+= 1;
                }

                if(obj) {
                    position= broke.storages.localStorage.position[kwargs.pk];
                    dbTable[position]= kwargs;
                    obj= kwargs;
                } else {
                    kwargs._meta= {
                    ...
                    broke.storages.localStorage.index[tableName][kwargs.pk]= kwargs;
                    position= dbTable.push(kwargs);
                    broke.storages.localStorage.position[kwargs.pk]= position-1;
                }
            ...
            ,'delete': function(tableName, pk){
                var
                    dbTable= broke.storages.localStorage.getTable(tableName)
                    ,position= broke.storages.localStorage.position[pk]
                ;
