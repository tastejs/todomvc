...
namespace $.$$ {
	...
	export class $todomvc extends $.$todomvc {
		...
		task( id : number , next? : $todomvc_task | null ) {
			const key = this.state_key( `mol-todos-${id}` )
			if( next === void 0 ) {
				return this.$.$mol_state_local.value<$todomvc_task>( key ) || { title : '' , completed : false }
			}
